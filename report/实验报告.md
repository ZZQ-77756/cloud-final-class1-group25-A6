...
- **成员信息**：
  ...
  - 成员3：()-()-文档编写与PPT制作
- **完成日期**：2026-01-04
- **代码仓库地址**：`https://github.com/ZZQ-77756/cloud-final-class1-group25-A6`
...
关键功能包括：编写 `Dockerfile` 实现镜像的标准化构建；使用 Shell 脚本（`run.sh`）实现“一键自动化部署”与版本切换；通过端口映射（Port Mapping）实现外部访问；利用卷挂载（Volume Mounting）实现数据的持久化与热更新。最终，我们成功实现了 v1 和 v2 双版本的无缝切换与管理，验证了容器技术的轻量级与敏捷性。
...
| 模块/任务 | 负责人 | 贡献说明 |
| --- | --- | --- |
| 环境搭建与代码仓库 | () | 完成 GitHub 仓库初始化、Git 权限配置、Dockerfile 编写 |
| 自动化脚本开发 | () | 编写 `run.sh`，实现一键部署与版本参数控制 |
| 验证与文档 | () | 完成 v1/v2 功能验证，编写实验报告与排错记录 |
...
1. **克隆仓库**：`git clone https://github.com/ZZQ-77756/cloud-final-class1-group25-A6.git`
2. **切换目录**：`cd cloud-final-class1-group25-A6`
3. **运行 v1**：`./run.sh`
4. **运行 v2**：`./run.sh v2`